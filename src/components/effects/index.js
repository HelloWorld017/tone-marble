...
class PoissonDenoisePass extends Pass {
  constructor(camera, textures, options = defaultPoissonBlurOptions) {
    ...
    this.renderTargetB = new WebGLRenderTarget(1, 1, renderTargetOptions); // give the textures of renderTargetA and renderTargetB names
  ...
  render(renderer) {
    for (let i = 0; i < 2 * this.iterations; i++) {
      ...
      this.fullscreenMaterial.uniforms["inputTexture"].value = i === 0 ? this.textures[0] : inputRenderTarget.texture;
